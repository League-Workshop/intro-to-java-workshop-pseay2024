...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	ScaryMaze() throws Exception {
		...
		String state = JOptionPane.showInputDialog("Easy, Medium, or Hard");
		if (state.equalsIgnoreCase("easy"))
		{
			maze = ImageIO.read(getClass().getResource("pixil-frame-0.png"));
		}else if (state.equalsIgnoreCase("medium"))
		{
			maze = ImageIO.read(getClass().getResource("pixil-frame-1.png"));
		}else if (state.equalsIgnoreCase("hard"))
		{
			maze = ImageIO.read(getClass().getResource("pixil-frame-2.png"));
		}
